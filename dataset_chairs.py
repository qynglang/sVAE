...
class LFW(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists('/nfs/h1/userhome/wdh-llx/workingdir/zsk/sVAE/Data/Face3/att/att1/'+str(self.imgs[idx])[61:-3]+'npy'):
            ...
            label[48:121]*=8
            l=label[48:121]
            iii=[1,2,3,9,10,12,14,16,17,19,28,29,30,34,35,37,38,43,46,59,60,63,65,67]
            label=np.hstack((label[0:10],label[24:34],l[iii]))
        else:
            label=np.zeros(44)
...
class chairs(Dataset):
    ...
    def __init__(self, 
                 ...
                 transform: Callable,
                **kwargs):
        self.data_dir = Path('Data/chair_new/chairs_new_1')       
        self.transforms = transform
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms is not None:
            img = self.transforms(img)
        #print('Data/chairs/att_1/'+str(self.imgs[idx])[25:-3]+'npy')
        #print(str(self.imgs[idx])[26:-3]+'npy')
        #if os.path.exists('Data/chair_new/chairs_att/'+str(self.imgs[idx])[26:-3]+'npy'):
        label=np.load('Data/chair_new/chairs_att_1/'+str(self.imgs[idx])[28:-3]+'npy')
            #print('1')
        #else:
            #label=np.zeros(31)
...
#       =========================  CelebA Dataset  =========================
        ...
        train_transforms = transforms.Compose([transforms.CenterCrop(592),
                                              transforms.Resize(self.patch_size),
                                              transforms.ToTensor(),])
        
        val_transforms = transforms.Compose([transforms.CenterCrop(592),
                                            transforms.Resize(self.patch_size),
                                            transforms.ToTensor(),])
        
        self.train_dataset = chairs(
            self.data_dir,
            split='train',
            transform=train_transforms,
            download=False,
